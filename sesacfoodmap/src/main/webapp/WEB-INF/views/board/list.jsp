		...
		<div class="col-lg-12">
			<div class="panel panel-default">
				<div class="panel-heading">
					<span id="ticket-span"> <label> <input
							id="ticketCheckbox" type="checkbox" name="ticket"
							class="table-top"
							<c:out value="${pageMaker.cri.ticket == true ? 'checked':'' }" />>
							<img id="ticketImg" src="/resources/assets/ticket-image.png"
							height="30px" />
					</label>
					</span> <span id="type-span"> <label> 식당 종류 <select
							id="typeSelect" name="type">
								<option value=""
									<c:out value="${pageMaker.cri.type == null ? 'selected':'' }" />>전체</option>
								<option value="한식"
									<c:out value="${pageMaker.cri.type == '한식' ? 'selected':'' }" />>한식</option>
								<option value="중식"
									<c:out value="${pageMaker.cri.type == '중식' ? 'selected':'' }" />>중식</option>
								<option value="양식"
									<c:out value="${pageMaker.cri.type == '양식' ? 'selected':'' }" />>양식</option>
								<option value="일식"
									<c:out value="${pageMaker.cri.type == '일식' ? 'selected':'' }" />>일식</option>
								<option value="아시안"
									<c:out value="${pageMaker.cri.type == '아시안' ? 'selected':'' }" />>아시안</option>
								<option value="카페"
									<c:out value="${pageMaker.cri.type == '카페' ? 'selected':'' }" />>카페</option>
								<option value="분식"
									<c:out value="${pageMaker.cri.type == '분식' ? 'selected':'' }" />>분식</option>
								<option value="기타"
									<c:out value="${pageMaker.cri.type == '기타' ? 'selected':'' }" />>기타</option>
						</select>
					</label>
					</span>
						...
						<div class="col-lg-12">
							...
							<form id='searchForm' action="/board/list" method='get'>
								<select name='searchType'>
									...
									<option value="TR"
										<c:out value="${pageMaker.cri.searchType == 'TR' ? 'selected':'' }" />>제목
										or 음식점</option>
									<option value="TW"
										<c:out value="${pageMaker.cri.searchType == 'TW' ? 'selected':'' }" />>제목
										or 작성자</option>
									<option value="TWR"
										<c:out value="${pageMaker.cri.searchType == 'TWR' ? 'selected':'' }" />>제목
										or 음식점 or 작성자</option>
